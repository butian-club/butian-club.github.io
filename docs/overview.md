...
[成员目录](./members.md) 审批与管用户，到 [部门](./departments.md) 管部门——
详见下表。
:::
...
| 成员：审批 / 改角色 / 启停 / 分部门 | [成员目录](https://gfssm.butian.club/members) 页面 | [成员目录](./members.md) |
...
- 真正执行改角色、审批、发布、删除等动作时，服务端会**逐个二次校验**管理员权限——
  不是「打开了页面就能为所欲为」。
...
1. 到 [成员目录](./members.md) 把积压的**待审批账号**处理掉，并给成员**分配部门**。
