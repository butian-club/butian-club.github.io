...
| 角色 | 名称 | 说明 |
| --- | --- | --- |
| `ADMIN` | 管理员 | 拥有全部管理能力：审批、成员、部门、任务、工单、文档、公告等管理入口**内联在各功能页面**上，系统级设置在后台 `/admin`（站点设置、倒计时、附件、操作日志） |
...
| `groups:manage` | 管理部门（创建、编辑、归档、分配成员） | ✅ | — |
| `tasks:view` | 查看任务 | ✅ | ✅ |
...
管理员在此之上额外拥有全部管理能力：审批、成员管理、部门、任务与工单的管理、
...
- **功能页面上的管理入口**（如任务的「新建」、成员的「审批」）对成员**不显示**；即便绕过界面
